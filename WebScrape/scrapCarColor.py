...
class CarColorScraper:
    def __init__(self):
        ...
        self.url = self.config.links[1]  # Color decoding website
        self.batch_size = 50  # Increased batch size for efficiency
        self.concurrent_requests = 10  # Run 10 Playwright pages in parallel
    ...
    async def get_color(self, page, vin):
        """Scrape vehicle color information asynchronously."""
    ...
    async def process_vins(self, vin_list):
        """Process multiple VINs asynchronously using multiple Playwright pages."""
        results = []
    ...
    def store_data(self, data):
        """Store data in the carColor table using batch inserts."""
        try:
    ...
    async def run(self):
        """Main function to fetch VINs, scrape data, and store results."""
        while True:
            df = self.fetch_vins()
            if df is not None and not df.empty:
                ...
                break  # Stop execution when no new VINs are left
