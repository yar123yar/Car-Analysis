...
class CarInfoScraper:
    def __init__(self):
        ...
        self.batch_size = 20  # Increased batch size for parallel execution
    ...
    async def get_info(self, page, vin, year):
        ...
        try:
            ...
            await page.wait_for_selector(".col-md-6", timeout=3000)  # More efficient than wait_for_timeout
            ...
            for text in panel_bodies.split('\n'):
                if "Transmission Style:" in text:
                    ...
                    break  # Stop looping after finding the transmission

            # Set transmission type based on availability
    ...
    async def process_vins(self, vin_list, year_list):
        """Process VINs in parallel and store after each batch."""
        results = []
        ...
        async with async_playwright() as p:
            ...
            for vin, year in zip(vin_list, year_list):
                ...
                if len(tasks) >= self.batch_size:
                    batch_results = await asyncio.gather(*tasks)  # Run tasks in parallel
                    results.extend(batch_results)
                    self.store_data(batch_results)  # Store after each batch
    ...
    def store_data(self, data):
        """Store new data in the carInfo table using bulk insert."""
        try:
    ...
    async def run(self):
        """Main function to fetch VINs, scrape data, and store it."""
        df = self.fetch_vins()
